...
const modes = [
  ...
  {
    id: "ingredients",
    label: "Generate Recipe Based on Ingredients",
    icon: "chefhat",
  },
...
const Ai = () => {
  ...
  // Load user data on component mount (no chat creation)
  useEffect(() => {
    loadUserData();
    // Check if there's an existing chat ID in localStorage or URL params
    loadExistingChat();
  }, []);
  ...
  const loadExistingChat = async () => {
    try {
      ...
      // Check for chat ID in localStorage or URL params
        ...
        if (chatData) {
          ...
          for (let i = 0; i < messages.length; i += 2) {
            ...
            if (
              ...
              if (
                aiMessage.recipeData &&
                typeof aiMessage.recipeData === "object"
              ) {
                formattedOutput = formatRecipeDisplay(
                  aiMessage.recipeData,
                  aiMessage.type || "idea"
                );
              }
              ...
              const chatEntry = {
                ...
                restrictions:
                  aiMessage.dietaryRestrictions?.join(", ") || "Previous Chat",
      ...
      // Continue without loading existing chat
    } finally {
  ...
  // Function to start a new chat session (clear current chat and messages)
  const startNewChat = async () => {
    try {
      ...
      setError("");
      setInput("");
      // Clear saved chat ID from localStorage
      localStorage.removeItem("currentChatId");
      // Note: New chat will be created on next submission
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create new chat if none exists (first time user submits)
      if (!chatId) {
        try {
          const newChat = await createNewChat();
          chatId = newChat._id; // Use _id from the response
          // Chat ID is already saved in createNewChat function
        } catch (chatError) {
      ...
      if (selectedMode === "idea") {
        ...
        if (recipeId) {
          ...
          response = await aiService.adaptExistingRecipe(
            chatId,
            null,
            input,
            recipeId
          ...
          setError(
            "Please provide either a recipe ID or original recipe details to adapt"
          );
          setIsLoading(false);
          return;
        }
      ...
      if (recipeData && typeof recipeData === "object") {
        ...
        formattedOutput =
          response.response?.content || "Recipe generated successfully!";
      }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#FFF6E9] via-[#FFDCA9] to-[#FF7F3F] dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 px-4 pt-10 pb-40 relative">
      ...
      {!output && !isLoadingChat && activeChats.length === 0 && (
        ...
        >
          ...
          >
            ...
            {activeChats.map((chat, idx) => (
              <div
                key={chat.timestamp}
                className="flex flex-col items-center w-full animate-fade-in-scale animate-expand-card"
              >
        ...
        >
          ...
          >
            ...
            {currentChat && !isLoadingChat && (
              <div className="w-full mb-2 bg-green-100 dark:bg-green-900/30 border border-green-400 text-green-700 dark:text-green-300 px-4 py-2 rounded text-sm">
                💬 Chat Active - ID: {currentChat._id?.slice(-8) || "Unknown"}
                {activeChats.length > 0 &&
                  ` (${activeChats.length} messages loaded)`}
              </div>
            )}
                  ...
                  <div key={day}>
                    ...
                    <div className="flex flex-col gap-4">
                      {items.map((item, i) => (
                        <div
                          key={item.idx}
                          className={`bg-[#FFF6E9] dark:bg-gray-800 border border-[#FFDCA9] dark:border-orange-400 rounded-2xl shadow p-4 animate-fade-in-item cursor-pointer ${
                        ...
                        >
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-[#FF7F3F] dark:text-orange-400 font-bold">
                              {item.mode === "idea" ? "Recipe Idea" : "Recipe"}
                            </span>
